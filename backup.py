...
#TODO Adjust logging to use this

import prettylogging
LOGGER_NAME = 'rpi_backup'
LOG_OUTPUT_PATH = './logs/logs.txt'
VERSION = '1.8'
...
class rpi_backup():
    def __init__(self):
        ...
        init(autoreset=True)  # Set auto-reset to True else color would be forwarded to the next print statement
    ...
    def setup(self):
        ...
        log.info('\n\nAttempting setup with supplied credentials...')

        self.check_networks_drive_credentials(credentials_dict)
        create_cifs_drive(credentials_dict['-networkpath'], mnt_path, credentials_dict['-username'],
    ...
    def run_backup(self, backup_type = None):
        ...
        if os.path.exists(f'{self.script_directory}/image-utils/image-backup') is False:
        ...
        if os.path.exists(f'/mnt/backups/{hostname}') is False:
            log.info(f"Creating Directory {hostname}")
